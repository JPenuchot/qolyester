...
#ifndef QOLYESTER_GRA_GRAPH_HXX
# define QOLYESTER_GRA_GRAPH_HXX 1
...
namespace olsr {
  ...
  namespace gra {
    ...
    AdjInfo::AdjInfo(const address_t& ep1, const address_t& ep2,
		     float loss, const timeval_t& delay,
		     const timeval_t& lbegin, const timeval_t& lend)
      : _ep1(ep1),
	_ep2(ep2),
	_loss(loss),
	_delay(delay),
    ...
    AdjInfo::operator==(const This& rhs) const {
      return _ep1 == rhs._ep1 && _ep2 == rhs._ep2;
    }

    const AdjInfo&
    AdjInfo::make_key(const address_t& ep1, const address_t& ep2) {
...
//       return This(x._ep2, x._ep1, x._loss, x._delay);
//     }
    ...
    AdjNode::AdjNode(const address_t& ep, const std::string& label)
      : _endpoint(ep),
	_label(label)
    {}
    ...
    AdjGraph::succ_iterator
    AdjGraph::succ_begin(nset_t::const_iterator pos) {
      assert(pos != _nset.end());
      apsetmap_t::const_iterator	x = _apset_map.find(&*pos);
      assert(x != _apset_map.end());
      return succ_iterator::build(x->second.begin());
    }

    AdjGraph::succ_iterator
    AdjGraph::succ_end(nset_t::const_iterator pos) {
      assert(pos != _nset.end());
      apsetmap_t::const_iterator	x = _apset_map.find(&*pos);
      assert(x != _apset_map.end());
      return succ_iterator::build(x->second.end());
    }
...
# endif
    ...
    AdjGraph::aset_t::const_iterator
    AdjGraph::insert_arc(const AdjInfo& a) {
      ...
      assert(n2 != _nset.end());
      // Insert the arc
      aset_t::iterator	p = _aset.insert(a);
      ...
      _apset_map[&*n1].insert(&*p);
      _apset_map[&*n2].insert(&*p);
      return p;
    }
